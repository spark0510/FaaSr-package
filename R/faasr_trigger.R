  ...
  if (length(invoke_next) == 0) {
    ...
    for (invoke_next_function in invoke_next) {
      ...
      switch(next_server_type,
        ...
        "OpenWhisk"={
          faasr_trigger_ow(faasr, next_server, invoke_next_function)
        },
				
       	# if AWS Lambda - use Lambda API
        "Lambda"={
          faasr_trigger_ld(faasr, next_server, invoke_next_function)
        },

        # if GitHub Actions - use GH Actions
        "GitHubActions"={
          faasr_trigger_gh(faasr, next_server, invoke_next_function)
    	  }

        #More to be developed
        # if Microsoft Azure
        #"Microsoft Azure"={
          #faasr_trigger_az(faasr)
        #},

        # if Google cloud
        #"Google Cloud"={
          #faasr_trigger_gc(faasr)
        #}
      )
    }
  }
...
faasr_trigger_ow <- function(faasr, next_server, invoke_next_function){
...
faasr_trigger_ld <- function(faasr, next_server, invoke_next_function){
  ...
  if (response$Status[1] == 202) {
    succ_msg <- paste0('{\"faasr_trigger\":\"Lambda: Successfully invoked: ', faasr$FunctionInvoke, '\"}\n')
    cat(succ_msg)
    faasr_log(succ_msg)
  } else {
    err_msg <- paste0('{\"faasr_trigger\":\"Lambda: Error invoking: ', faasr$FunctionInvoke, ' - ', response$Status[1], '\"}\n')
    cat(err_msg)
    faasr_log(err_msg)
  }
}
...
faasr_trigger_gh <- function(faasr, next_server, invoke_next_function){
  ...
  if (status_code(response) == 204) {
    succ_msg <- paste0('{\"faasr_trigger\":\"GitHub Actions: Successfully invoked: ', faasr$FunctionInvoke, '\"}\n')
    cat(succ_msg)
    faasr_log(succ_msg)
  } else if (status_code(response) == 401) {
	  err_msg <- paste0('{\"faasr_trigger\":\"GitHub Actions: Error invoking: ', faasr$FunctionInvoke, ' - Authentication failed, check the credentials\"}\n')
    cat(err_msg)
    faasr_log(err_msg)
	} else if (status_code(response) == 404) {
	  err_msg <- paste0('{\"faasr_trigger\":\"GitHub Actions: Error invoking: ', faasr$FunctionInvoke, ' - Cannot find the destination, check the repo name: \"',repo,'\" and workflow name: \"',workflow_file,'\"}\n')
    cat(err_msg)
    faasr_log(err_msg)
	} else if (status_code(response) == 422) {
	  err_msg <- paste0('{\"faasr_trigger\":\"GitHub Actions: Error invoking: ', faasr$FunctionInvoke, ' - Cannot find the destination, check the branch: ', faasr$FunctionInvoke,'\"}\n')
    cat(err_msg)
    faasr_log(err_msg)
	} else {
	  err_msg <- paste0('{\"faasr_trigger\":\"GitHub Actions: Error invoking: ', faasr$FunctionInvoke, ' - unknown error happens when invoke next function\"}\n')
    cat(err_msg)
    faasr_log(err_msg)
  }
}
...
#faasr_trigger_az <- function(faasr){
  
#}


#faasr_trigger_gc <- function(faasr){
